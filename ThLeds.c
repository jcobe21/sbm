...
/*----------------------------------------------------------------------------
 * CUIDADO en verdad los leds no necesitan ser manejados por un hilo
 *  es un código demasiado simple
 *---------------------------------------------------------------------------*/
...
//    pinB14.counter = 0;
  ...
  */
  ...
  Init_Timer();                                                                 //solo quiero inicializar el timer una vez
  
  while (1) {
    
     statusLeds = osThreadFlagsWait(0x1000, osFlagsWaitAny, osWaitForever);
    
    if (statusLeds == 0x1000){
      osTimerStart(timer, 50U);                                   //aqui inicializo timer cada vez que me llegue la flag
    }
